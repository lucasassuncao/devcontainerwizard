...
func newInitCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return runInitE(cmd, list, force, template)
		},
	}
...
func runInitE(cmd *cobra.Command, list, force bool, template string) error {
	if list {
	...
	fmt.Fprintf(cmd.OutOrStdout(), "Created config.yaml from template %q.\n\nNext: devcontainerwizard edit -c config.yaml\n", template)
	return nil
}
