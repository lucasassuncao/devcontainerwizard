...
func TestLoadDocument_missing(t *testing.T) {
	...
	doc, err := edit.LoadDocument(path)
	if err != nil {
		t.Fatalf("LoadDocument on missing file: unexpected error %v", err)
	}
	if doc == nil {
...
func TestLoadDocument_crlf(t *testing.T) {
...
func TestDocument_BlockContent(t *testing.T) {
	doc, err := edit.NewDocument([]byte("name: mydev\nimage: ubuntu:22.04\n"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestDocument_RemoveNotFound(t *testing.T) {
	doc, _ := edit.NewDocument([]byte("name: mydev\n"))
...
func TestDocument_ReplaceRawInvalid(t *testing.T) {
	...
	doc, err := edit.NewDocument(original)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDocument_HistoryCapsAt50(t *testing.T) {
	...
	if count != 50 {
		t.Errorf("expected 50 undos available, got %d", count)
	}
}
...
func TestDocument_UnknownKeys(t *testing.T) {
	doc, _ := edit.NewDocument([]byte("name: mydev\ncustomization: bad\n"))
	unknown := doc.UnknownKeys()
	if len(unknown) != 1 || unknown[0] != "customization" {
		t.Errorf("expected [customization], got %v", unknown)
	}
}

func TestDocument_Conflicts(t *testing.T) {
	doc, _ := edit.NewDocument([]byte("name: mydev\nimage: ubuntu:22.04\nbuild:\n  dockerfile: Dockerfile\n"))
	conflicts := doc.Conflicts()
	if len(conflicts) == 0 {
		t.Error("expected mutual-exclusion conflict for image+build, got none")
	}

	doc2, _ := edit.NewDocument([]byte("name: mydev\nimage: ubuntu:22.04\n"))
	if c := doc2.Conflicts(); len(c) != 0 {
		t.Errorf("expected no conflicts for image alone, got %v", c)
	}
}
