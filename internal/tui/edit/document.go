...
// historyLimit caps the undo stack. Renamed from maxHistory in model.go to
// avoid redeclaration in the same package during the migration; model.go's
// constant is removed when the Model migrates to use the Document.
...
// LoadDocument reads a YAML file from path. A non-existent file is not an
// error — the returned Document is empty, dirty=false, and Save writes to path.
func LoadDocument(path string) (*Document, error) {
	...
	return &Document{path: path, raw: raw, blocks: blocks}, nil
}

// NewDocument builds a Document from raw bytes. Intended for tests and
// in-memory use; the resulting document has no file path.
func NewDocument(raw []byte) (*Document, error) {
	...
	return &Document{raw: raw, blocks: blocks}, nil
}
...
func (d *Document) Insert(snippet string) error {
	newRaw, err := InsertBlock(d.raw, snippet)
	if err != nil {
		return err
	}
	d.snapshot()
	d.raw = newRaw
	blocks, err := ParseBlocksFromBytes(newRaw)
	if err != nil {
...
func (d *Document) Replace(key, snippet string) error {
	...
	inserted, err := InsertBlock(removed, snippet)
	if err != nil {
		return err
	}
	d.snapshot()
	d.raw = inserted
	blocks, err := ParseBlocksFromBytes(inserted)
	if err != nil {
...
func (d *Document) UnknownKeys() []string {
	return ValidateKnownKeys(d.raw)
}

// Conflicts returns human-readable messages for mutual-exclusion violations.
func (d *Document) Conflicts() []string {
	return ValidateMutualExclusions(d.blocks)
}

// Save writes the current raw to disk at d.path with mode 0600 and clears dirty.
// Returns an error if d.path is empty (Document created with NewDocument).
func (d *Document) Save() error {
	if d.path == "" {
		return fmt.Errorf("document has no path; LoadDocument requires a path")
	}
